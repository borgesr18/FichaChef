...
export const POST = withErrorHandler(async function POST(request: NextRequest) {
  ...
  for (const config of configEstoque) {
    ...
    if (saldoAtual <= (config.limiteEstoqueBaixo || 0)) {
      ...
      const notificacaoExistente = await withConnectionHealthCheck(async () => {
        return await withDatabaseRetry(async () => {
      ...
      if (!notificacaoExistente) {
        const notificacao = await withConnectionHealthCheck(async () => {
          return await withDatabaseRetry(async () => {
...
async function obterNomeItem(itemId: string, itemTipo: string): Promise<string> {
  if (itemTipo === 'insumo') {
    const insumo = await withDatabaseRetry(async () => {
      return await prisma.insumo.findFirst({ where: { id: itemId } })
    })
    return insumo?.nome || 'Item não encontrado'
  } else {
    const produto = await withDatabaseRetry(async () => {
      return await prisma.produto.findFirst({ where: { id: itemId } })
    })
